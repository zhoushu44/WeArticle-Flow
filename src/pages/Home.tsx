...
export default function Home() {
  ...
  const generateArticle = async (title: string, selectedTitle: number | null) => {
    setGenerationBusy(true); setGenerationError('')
    try {
      ...
      set({ selectedTitle, stage: 'sections', sectionIndex: 0, confirmedSections: Array(4).fill(false), html: payload.html, imageCandidates: {}, selectedImages: {}, uploadedImages: {} })
  ...
  const generateCurrentImages = async () => {
    ...
    try {
      ...
      const prompts = imageSlots.map(({ slot, label, ratio }) => {
        const visualRole = ratio ? '提炼主题中最吸引人的冲突、变化、情绪或核心发现，用一个强烈、易懂的视觉隐喻表现，形成前景主体与背景环境的明显反差' : `围绕${label}表现主题如何影响真实的人、场景、认知或决策，画面必须有一个明确主体和可读的叙事关系`
        const textRule = ratio ? `画面必须加入清晰、准确、可读的简体中文主标题“${titleText}”，放在上方或左侧留白区，使用现代粗体排版，高对比度，不能改字、漏字或生成乱码；可加一行不超过 10 字的副标题。` : `画面加入一个简短的简体中文信息标签“${label}”，字要清晰可读，避免乱码；不要添加其他文字。`
        return { slot, prompt: `为微信公众号文章创作高点击率配图，先根据内容判断是产品、科普、教育、商业、生活、文化、人物、事件还是研究主题，并匹配视觉语言。主题：${product}。文章标题：${titleText}。参考内容：${research}。${visualRole}。采用与主题匹配的真实摄影、编辑插画或信息可视化风格，构图大胆但留白充足，主体在移动端小图中仍清晰，使用与主题气质相符的色彩，加入尺度、光线、前后对比或场景冲突来制造好奇心，避免与主题无关的实验室、芯片、网络线和泛科技背景。${textRule}不要复刻论文图表，不要 logo、水印、界面截图或无意义字母数字。${ratio ? `横版 ${ratio}` : '方形构图'}，适合公众号文章。` }
      })
